...
func (h *ServerHandler) CreateListener(address string) error {
	...
	go func() {
		log.Info("wss endpoint : ", endpoint)
		err := http.ListenAndServeTLS(h.address, config.Current.Security.CertPem, config.Current.Security.KeyPem, nil)
		if err != nil {
			_ = log.Error("wss server stopped : ", err)
		}
	}()
	return nil
}
